define([
  ...
  return declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin], {
      ...
      if (files && files.length === 1) {
        ...
        array.some(types, function(type) {
          if (type.indexOf('text/uri-list') !== -1) {
			...
			url = url.replace(/^http:\/\//i, 'https://');//added by matt to replace http with https - Matt//
    ...
    handleZip: function(file) {
      ...
      request({
        url: 'https://www.arcgis.com/sharing/rest/content/features/generate', //added https to make work on https://www.barr.com host - Matt//
        content: myContent,
        form: formData,
        handleAs: 'json'
      }).then(
